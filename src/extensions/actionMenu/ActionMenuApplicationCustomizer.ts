...
> {
  ...
  private checkElement(item: ItemMenuSetting): void {
    ...
    // },100);
    if (jQ("#O365_SubLink_Change_The_Look").length || jQ("#O365_SubLink_ShellSignout").length) {
      let _menu:string=find(self.items,(it)=> {return (it===item.Id);});
      if(_menu===undefined) {
  ...
  private insertInMenu(item: ItemMenuSetting): void {
    ...
    if (item.MenuGroupType === MenuGroup.AdminMenu) {
      document.getElementById("O365_SubLink_ShellSignout").parentElement.appendChild(ncAnchor);
    } else {
      document.getElementById("O365_SubLink_Change_The_Look").parentElement.appendChild(ncAnchor);
     }
